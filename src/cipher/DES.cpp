...
void DES :: generate_key(string key){ // bit string
    string pc_string; // string generated by Permutation_choice_1_
    ...
    string left = pc_string.substr(0,28);
    string right = pc_string.substr(28);
...
string DES :: circular_shift_one(string str){
    char ch = str[0];
    int size = str.size();
...
string DES :: circular_shift_two(string str){
...
int DES :: BinaryToDecimal(const string& binary){
    int res = 0;
    for(auto x : binary){
        res *= 2;
        res += x - '0';
...
string  DES :: DecimalToBinary(int decimal){
    string binary;
    int d;
    while(decimal){
        d = decimal % 2;
...
string  DES :: XorString(string A, string B){
    string res;
    int size = A.size();
    int d;
...
string DES :: f(string message, string key){
    ...
    string xored = XorString(expanded, key);

    // now compress 48-bit xored to 32-bit using S_box_
    string compressed;
    for(int i = 0; i < 8; i++){
        string r = xored.substr(i*6,1) + xored.substr(i*6 + 5,1);
        string c = xored.substr(6 * i + 1, 4);
...
string DES :: Encrypt(string message){
    ...
    string right =  ip.substr(32);
    string l, r, black_box; // two string buffer
    for(int i = 0; i < 16; i++){
        l = right;
        black_box = f(right, subkey_[i]);
...
string  DES :: Decrypt(string message){
    reverse_key();
